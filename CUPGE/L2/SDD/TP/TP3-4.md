...
Voici le pseudo-alogrithme du parcours en prodonfeur d'abord (**DFS** : *Depth-First Search*) : 
```
**Initialisation**
 Créer une pile vide
 Ajouter la case de départ à la pile
 Créer un ensemble (ou un tableau de booléens) pour marquer les cases visitées

**Boucle principale** 
Tant que la pile n'est pas vide
	Extraire la case actuelle (le sommet de la pile)
	Si cette case est la sortie, l'algorithme s'arrête (Victoire !)
	Si la case n'a pas encore été visitée
		La marquer comme visitée
		Pour chaque voisin accessible non visité, l'ajouter à la pile
```
