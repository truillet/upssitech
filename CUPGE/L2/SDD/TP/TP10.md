...
Le choix du port n’a pas d’importance particulière à partir du moment où le port sélectionné est libre (i.e. non utilisé par un autre service[[1]](#footnote-1)). Typiquement (et par défaut dans les implémentations d’ivy), le port ‘2010’ est utilisé mais on peut utiliser plusieurs bus à divers ports en même temps.
...
![VcXsrv Windows X Server / Wiki / Home](data:image/png;base64...)Nous allons utiliser un agent déjà développé en java. Avant d’effectuer cela, assurez-vous qu’un serveur X est lancé (pas de problèmes normalement sous Unix ou MacOS ou Windows 11 avec WSL2).

Sous Windows 10, sous pouvez utiliser le serveur X **VcXsrv**(https://sourceforge.net/projects/vcxsrv) [[2]](#footnote-2)
...
![](data:image/png;base64...) Une fenêtre graphique vide doit apparaître sur votre écran.
...
1. Les ports 1 à 1024 sont réservés par le système pour les services « bien connus » (well known ports) comme http (port 80), ftp (port 21), … [↑](#footnote-ref-1)
2. Voir **https://medium.com/@japheth.yates/the-complete-wsl2-gui-setup-2582828f4577** [↑](#footnote-ref-2)
