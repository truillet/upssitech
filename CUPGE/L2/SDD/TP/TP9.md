...
Il faut installer tout d’abord le JDK Java et instancier la variable **JAVA\_HOME** (exemple ci-dessous pour Linux) :

sudo apt install openjdk-17-jdk-headless

export JAVA\_HOME=/usr/lib/jvm/java-17-openjdk-amd64[[1]](#footnote-1)
...
public static void main(String[] args) { HelloJNI h = new HelloJNI();
...
Dans un 2ème temps, compilons le fichier (attention à bien utiliser l’option **-d**)

javac -d . HelloJNI.java

Normalement, le fichier HelloJNI.class est créé dans le sous-répertoire : fr/ut3
...
javac -h . HelloJNI.java (pour Java 8 et supérieur) qui va générer le code fr\_ut3\_HelloJNI.h

Code généré :

/\* DO NOT EDIT THIS FILE - it is machine generated \*/ #include <jni.h>

/\* Header for class fr\_ut3\_HelloJNI \*/

#ifndef \_Included\_fr\_ut3\_HelloJNI #define \_Included\_fr\_ut3\_HelloJNI #ifdef cplusplus
...
JNIEXPORT jstring JNICALL Java\_fr\_ut3\_HelloJNI\_getGreetings (JNIEnv \*, jclass, jstring);
...
Dernière étape, générons la librairie dynamique (**.so** sous Unix, **.dll** sous windows) grâce aux deux commandes suivantes[[2]](#footnote-2) :
...
| long | jlong | 64 bits signés. Valeur entre -263 et 263-1 |
| short | jshort | 16-bit signés. Valeur entre -215 et 215–1 |
| boolean | jboolean | 8 bits non signés. *true* et *false* |
...
* une fonction **creerListe()** qui créé une liste
...
* Et une fonction **compter()** qui compter le nombre d’éléments de la liste

Vous écrirez enfin un programme java appelant ces fonctions natives (en utilisant ou non une interface graphique)

1. Sous MacOS, lancer la commande /usr/libexec/java\_home pour trouver le chemin vers la JDK [↑](#footnote-ref-1)
2. Sous MacOS, remplacer "$JAVA\_HOME/include/linux" par "$JAVA\_HOME/include/darwin" [↑](#footnote-ref-2)
